...
// Per-terminal settings for agent windows only: terminal-helpers/<terminal>/.
export const HELPERS_DIR = join(dirname(fileURLToPath(import.meta.url)), "..", "terminal-helpers");

// The user's own kitty.conf (kitty stops reading it once --config is given, so name it first).
export function userKittyConf(env = process.env) {
  const dir = env.KITTY_CONFIG_DIRECTORY || join(env.XDG_CONFIG_HOME || join(homedir(), ".config"), "kitty");
  const f = join(dir, "kitty.conf");
  return existsSync(f) ? f : null;
}

// kitty: your kitty.conf, then terminal-helpers/kitty/pi.conf (Ctrl+click links, SUPER+C / Ctrl+Shift+A /
// Shift+Enter for pi, select-to-copy), so those apply to agent windows and nowhere else.
// "terminalHelpers": false in the config skips pi.conf.
function kittyArgs({ cls, cwd, helpers }) {
  const pi = join(HELPERS_DIR, "kitty", "pi.conf");
  const conf = [];
  if (helpers && existsSync(pi)) {
    const mine = userKittyConf();
    if (mine) conf.push("--config", mine);
    conf.push("--config", pi);
  }
  return ["kitty", `--class=${cls}`, ...conf, `--directory=${cwd}`, "--"];
}

const KNOWN = {
  foot: ({ cls, title, cwd }) => ["foot", `--app-id=${cls}`, `--title=${title}`, `--working-directory=${cwd}`, "--"],
  // No --title for kitty: it pins the title, so Pi could never show
  // "π - <name> - <folder>" there. Select-to-copy etc. come from terminal-helpers/kitty/pi.conf.
  kitty: kittyArgs,
...
export function terminalCommand(cfg, { title = "pi", cwd }) {
  ...
  return KNOWN[resolveTerminal(cfg)]({ cls: AGENT_CLASS, title, cwd, helpers: cfg.terminalHelpers !== false });
}

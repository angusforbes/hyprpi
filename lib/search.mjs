...
function runPi(prompt, { model, timeoutMs, pi }) {
  return new Promise((resolve, reject) => {
    ...
    let out = "", err = "";
    const t = setTimeout(() => { child.kill("SIGKILL"); reject(new Error(`AI search timed out after ${timeoutMs / 1000}s`)); }, timeoutMs);
    ...
    child.on("close", (code) => {
      ...
      else reject(new Error(`AI search failed (${code}): ${err.trim().split("\n").slice(-2).join(" ").slice(0, 300)}`));
    });

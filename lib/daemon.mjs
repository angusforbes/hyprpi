...
export async function runDaemon({ env = process.env, log = (...a) => console.error(new Date().toISOString(), ...a) } = {}) {
  ...
  function roomSources(room, only = null, { closed = true } = {}) {
  ...
  const methods = {
    ...
    ask: async (p) => {
      ...
      const acts = activityTail(room).filter((e) => (!only || only.includes(e.agent?.id)) && (closed || agents.has(e.agent?.id)));
    ...
    search: async (p) => {
      ...
      const sources = roomSources(room, only, { closed: p.closed !== false });
      const t0 = Date.now();
      if (p.mode === "ai") {

...
export async function runDaemon({ env = process.env, log = (...a) => console.error(new Date().toISOString(), ...a) } = {}) {
  ...
  function recordActivity(a, kind, text, extra = {}) {
    if (cfg.activity === false || !a || !a.room || a.parked) return;
    text = gist(text, 200);
    if (!text) return;
    const room = safeRoom(a.room);
  ...
  const methods = {
    ...
    "agent.activity": (p, conn) => {
      ...
      if (cfg.activityTools === false) return { recorded: 0 };
      const items = Array.isArray(p.items) ? p.items.slice(0, 10) : [];
      for (const it of items) recordActivity(a, "tool", it?.text);

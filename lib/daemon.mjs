...
export async function runDaemon({ env = process.env, log = (...a) => console.error(new Date().toISOString(), ...a) } = {}) {
  ...
  function view(a) {
    return {
      ...
      focused: !!a.focused, seen: !(a.status === "done" || a.status === "blocked") || !!a.seen,
    };
  }
  ...
  function syncUnseen() {
    try {
      ...
      const want = new Set([...agents.values()].filter((a) => a.address && (a.status === "done" || a.status === "blocked") && !a.seen).map((a) => normAddr(a.address)));
  ...
  const methods = {
    ...
    "agent.update": (p, conn) => {
      ...
      if (p.status && ["idle", "working", "blocked", "done"].includes(p.status)) {
        ...
        if ((status === "done" || status === "blocked") && prev !== status) a.seen = false;
    ...
    "agent.seen": (p, conn) => {
      ...
      if ((a.status === "done" || a.status === "blocked") && !a.seen) { a.seen = true; remember(a); pushAgents(); }

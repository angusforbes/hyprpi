...
export async function runDaemon({ env = process.env, log = (...a) => console.error(new Date().toISOString(), ...a) } = {}) {
  ...
  const stopEvents = hypr.subscribe((ev) => {
    if (/^(openwindow|closewindow|movewindow|movewindowv2|workspace|workspacev2|focusedmon|activewindow|activewindowv2|changefloatingmode)$/.test(ev)) refreshSoon();
  }, env);
  ...
  // --------------------------------------------------------------- methods ---
  const methods = {
    ping: () => ({ ok: true, version: VERSION, socket: SOCK, pid: process.pid }),
    ...
    // room "" = the current world's room (the widget then follows the world).
    "ui.open": (p) => {
      const room = p.room ? safeRoom(p.room) : "";
      let n = 0;
      for (const c of conns) if (c.windows) { send(c, "open", { room, toggle: !!p.toggle }); n++; }
      return { room: room || roomForWorkspace(activeWs) || "A", delivered: n };
    },

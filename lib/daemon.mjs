...
export async function runDaemon({ env = process.env, log = (...a) => console.error(new Date().toISOString(), ...a) } = {}) {
  ...
  function view(a) {
    return {
      ...
      focused: !!a.focused, seen: a.status !== "done" || !!a.seen,
    };
  }
  ...
  function syncUnseen() {
    try {
      ...
      const want = new Set([...agents.values()].filter((a) => a.address && a.status === "done" && !a.seen).map((a) => normAddr(a.address)));
  ...
  const methods = {
    ...
    "agent.update": (p, conn) => {
      ...
      if (p.status && ["idle", "working", "blocked", "done"].includes(p.status)) {
        ...
        // A finish is unseen until you visit the agent (already there = seen).
        if (status === "done" && prev !== "done") a.seen = !!a.focused;
        // Herdr-style sounds, played by the room app: "done" when a turn finishes
        // (skipped if you're already looking at that agent's window), "request"
        // when an agent becomes blocked on you.
        // `quiet`: the caller makes its own sound (bonk's thud).
        if (prev === "working" && status === "done" && !a.focused) broadcastUi("sound", { sound: "done", agent_id: id, name: a.name, room: a.room });
    ...
    // Click inside an agent window (Hyprland hook): its \u2713 becomes seen.
    "agent.seen": (p) => {
      const addr = String(p.address || "").toLowerCase().replace(/^0x/, "");
      const a = [...agents.values()].find((x) => x.address && x.address.toLowerCase().replace(/^0x/, "") === addr);
      if (!a) return { seen: false };
      if (a.status === "done" && !a.seen) { a.seen = true; remember(a); pushAgents(); }

  ...
  const methods = {
    ...
    "agent.update": (p, conn) => {
      ...
      if (p.status && ["idle", "working", "blocked", "done"].includes(p.status)) {
        ...
        // A finish is unseen until you click or type in the agent's window,
        // even if it was already focused.
        if (status === "done" && prev !== "done") a.seen = false;
    ...
    // (the agent's extension, by its connection): its \u2713 becomes seen.
    "agent.seen": (p, conn) => {

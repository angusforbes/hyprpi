  ...
  const server = net.createServer((sock) => {
    ...
    sock.on("close", () => {
      ...
      if (a && a.conn === conn) {
        ...
        a.conn = null;
        const drop = () => { if (!a.conn && agents.get(a.id) === a) { recordActivity(a, "left", "left"); agents.delete(a.id); remember(a); pushAgents(); } };
        const dead = () => { if (!a.pid) return false; try { process.kill(a.pid, 0); return false; } catch (e) { return e.code === "ESRCH"; } };

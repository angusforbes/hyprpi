...
export async function runDaemon({ env = process.env, log = (...a) => console.error(new Date().toISOString(), ...a) } = {}) {
  ...
  function remember(a) {
    registry[a.id] = {
      ...
      status: a.status || "idle", ack: a.ack !== false, pid: a.pid || 0,
    };
    saveRegistry();
  }
  ...
  // only: agent ids to restrict to (the room TUI's marked agents); their room-log posts are kept.
  function roomSources(room, only = null) {
    ...
    const log = roomLog(room).msgs.filter((m) => !keep || keep.has(m.author?.id)).map((m) => ({ eid: String(m.seq), role: m.author?.kind === "agent" ? "room" : "angus", ts: m.ts, text: `${m.author?.name || "?"}: ${m.text}` }));
  ...
  function view(a) {
    return {
      ...
      room: a.room || "", parked: !!a.parked, twin_of: a.twinOf || "", since: a.createdAt, active: a.lastActive,
  ...
  const pushAgents = debounce(() => { for (const a of agents.values()) pushSelf(a); syncUnseen(); broadcastUi("agents", { agents: agentList(), rooms: knownRooms(), active_room: roomForWorkspace(activeWs), active_workspace: activeWs }); }, 80);
  ...
  async function refreshWindows() {
    ...
    try {
      ...
      for (const a of agents.values()) {
        ...
        if (a.parked) { if (a.room) { a.room = ""; changed = true; } }
        else {
  ...
  const methods = {
    ...
    "agent.hello": (p, conn) => {
      ...
      const a = prev || {
        ...
        name: reg.name || "", nameMarkup: reg.nameMarkup || "", icon: reg.icon || "", color: reg.color || "", topic: reg.topic || "", topicKey: reg.topicKey || "", homeRoom: cfg.follow ? null : reg.homeRoom || null,
      };
    ...
    "agent.update": (p, conn) => {
      ...
      if (p.resync && p.status && ["idle", "working", "blocked", "done"].includes(p.status)) {
        ...
        // `quiet`: the caller makes its own sound (bonk's thud).
        ...
        if (prev === "working" && status === "done") broadcastUi("sound", { sound: "done", agent_id: id, name: a.name, room: a.room });
    ...
    "room.post": (p, conn) => {
      ...
      if (a) {
        ...
        const m = appendMessage(a.room, { author: { kind: "agent", id: a.id, name: displayName(a), icon: a.icon, color: a.color, markup: a.nameMarkup || "" }, text });
    ...
    "agent.prompt": (p) => {
      ...
      recordActivity(agent, "prompt", `Angus → ${displayName(agent)}: ${gist(text)}`);
    ...
    talk: (p, conn) => {
      ...
      if (delivered.length) recordActivity(from, mode, `${mode === "demand" ? "asks" : "to"} ${delivered.join(", ")}: ${gist(text)}`, { to: delivered });
    ...
    "talk.reply": (p, conn) => {
      ...
      recordActivity(a, "reply", `replies to ${sender ? displayName(sender) : "?"}: ${gist(text)}`, { to: sender ? [displayName(sender)] : [] });
    ...
    ask: async (p) => {
      ...
      const acts = activityTail(room).filter((e) => !only || only.includes(e.agent?.id));
      const sources = [...roomSources(room, only), activitySource(acts)];
      const t0 = Date.now();
    ...
    search: async (p) => {
      ...
      const sources = roomSources(room, only);
      const t0 = Date.now();
      if (p.mode === "ai") {
  ...
  const server = net.createServer((sock) => {
    ...
    sock.on("close", () => {
      ...
      if (a && a.conn === conn) {
        ...
        a.conn = null;
        setTimeout(() => { if (!a.conn && agents.get(a.id) === a) { recordActivity(a, "left", "left"); agents.delete(a.id); remember(a); pushAgents(); } }, 4000);
      }
    });
  });
  server.listen(SOCK, () => {

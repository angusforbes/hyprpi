...
export async function runDaemon({ env = process.env, log = (...a) => console.error(new Date().toISOString(), ...a) } = {}) {
  ...
  function roomSources(room, only = null) {
    ...
    const log = roomLog(room).msgs.filter((m) => !keep || m.author?.kind === "human" || keep.has(m.author?.id)).map((m) => ({ eid: String(m.seq), role: m.author?.kind === "agent" ? "room" : "angus", ts: m.ts, text: `${m.author?.name || "?"}: ${m.text}` }));

...
export async function runDaemon({ env = process.env, log = (...a) => console.error(new Date().toISOString(), ...a) } = {}) {
  ...
  const methods = {
    ...
    "agent.update": (p, conn) => {
      ...
      if (p.status && ["idle", "working", "blocked", "done"].includes(p.status)) {
        const prev = a.status;
    ...
    ask: async (p) => {
      ...
      const sources = [...roomSources(room), activitySource(activityTail(room))];
      const t0 = Date.now();
    ...
    search: async (p) => {
      ...
      const sources = roomSources(room);
      const t0 = Date.now();
      if (p.mode === "ai") {

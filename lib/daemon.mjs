  ...
  async function refreshWindows() {
    ...
    try {
      ...
      for (const a of agents.values()) {
        ...
        // Visiting a finished agent's window marks its "done" as seen.
        if (a.focused && a.status === "done" && !a.seen) { a.seen = true; changed = true; }
  ...
  const methods = {
    ...
    "agent.update": (p, conn) => {
      ...
      if (p.status && ["idle", "working", "blocked", "done"].includes(p.status)) {
        const prev = a.status;
        a.status = p.status; a.lastActive = Date.now();
        // A finish is unseen until you visit the agent (already there = seen).
        if (p.status === "done" && prev !== "done") a.seen = !!a.focused;
        ...
        if (prev === "working" && p.status === "done" && !a.focused) broadcastUi("sound", { sound: "done", agent_id: id, name: a.name, room: a.room });
        else if (prev !== "blocked" && p.status === "blocked") broadcastUi("sound", { sound: "request", agent_id: id, name: a.name, room: a.room });
      }

...
export async function runDaemon({ env = process.env, log = (...a) => console.error(new Date().toISOString(), ...a) } = {}) {
  ...
  const pushAgents = debounce(() => { syncUnseen(); broadcastUi("agents", { agents: agentList(), rooms: knownRooms(), active_room: roomForWorkspace(activeWs), active_workspace: activeWs }); }, 80);
  ...
  // user messages; re-summarised only when the latest one changes. At most two
  // summaries run at once. Config: topics (default true), topicModel.
  const topicBusy = new Set();
  function refreshTopics() {
    if (cfg.topics === false) return;
    for (const a of agents.values()) {
      if (!a.session || topicBusy.has(a.id) || topicBusy.size >= 2) continue;
      const inp = topicInput(a.session);
      if (!inp || inp.key === a.topicKey) continue;
      topicBusy.add(a.id);
      summarize(inp.src, { pi: cfg.pi || "pi", model: cfg.topicModel || cfg.searchModel || "claude-haiku-4-5" })
        .then((topic) => {
          a.topicKey = inp.key; // don't retry the same input on failure
          if (topic && topic !== a.topic) { a.topic = topic; remember(a); pushAgents(); }
          else remember(a);
        })
        .finally(() => topicBusy.delete(a.id));
    }
  }
  const topicPoll = setInterval(refreshTopics, 20000);
  ...
  const methods = {
    ...
    "agent.hello": (p, conn) => {
      ...
      const a = prev || {
        ...
        name: reg.name || "", icon: reg.icon || "", color: reg.color || "", topic: reg.topic || "", topicKey: reg.topicKey || "", homeRoom: cfg.follow ? null : reg.homeRoom || null,
      };
      Object.assign(a, {
  ...
  const quit = () => { clearInterval(poll); clearInterval(topicPoll); stopEvents(); server.close(); try { fs.unlinkSync(SOCK); } catch {} process.exit(0); };

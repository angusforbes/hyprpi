...
export const COMMANDS = [
  ...
  ["/history", "/history N | all: how far back the stream and /ask go (N interactions = room messages or agent turns; default 200)"],
  ["/help", "this list"],
];
...
export function createSearch({ api, room, onChange, only = () => [], history = () => 200 }) {
  const s = {
    ...
    run(query, mode = s.mode) {
      ...
      onChange();
      a.call("search", { room: room(), query: q, mode, agents: ids.length ? ids : undefined, closed: false, room_log: false }, { timeoutMs: 180000 }).then((r) => {
        if (g !== s.gen) return;
    ...
    askQuestion(question) {
      ...
      const ids = only();
      a.call("ask", { room: room(), question: q, agents: ids.length ? ids : undefined, interactions: history() }, { timeoutMs: 120000 }).then((r) => {
        if (g !== A.gen) return;
...
export function helpRows(t) {
  ...
  return [
    ...
    `   ${t.dim("pane (Ctrl): ^↑↓ select stream item / result · PgUp PgDn · ^Home/End · ^/ stream → search → ask · ^S keyword ⇄ ai · ^F stream view")}`,

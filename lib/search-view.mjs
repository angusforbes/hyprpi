...
export function createSearch({ api, room, onChange, only = () => [] }) {
  const s = {
    ...
    run(query, mode = s.mode) {
      ...
      onChange();
      a.call("search", { room: room(), query: q, mode, agents: ids.length ? ids : undefined, closed: false }, { timeoutMs: 180000 }).then((r) => {
        if (g !== s.gen) return;
    ...
    askQuestion(question) {
      ...
      const ids = only();
      a.call("ask", { room: room(), question: q, agents: ids.length ? ids : undefined, closed: false }, { timeoutMs: 120000 }).then((r) => {
        if (g !== A.gen) return;

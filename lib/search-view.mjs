...
export function resultRows(r, selected, W, worldFg, t) {
  ...
  if (t.width(pre) > maxPre) { // keep the end of it (walk back from the match, linear)
    const cs = [...pre]; let w = 0, k = cs.length;
    while (k > 0) { const c = t.width(cs[k - 1]); if (w + c > maxPre - 1) break; w += c; k--; }
    pre = "…" + cs.slice(k).join("");
  }
...
// only formats what changed; hits are 1 row (2 with an AI reason).
...
export function searchRows(s, W, worldFg, t) {
  ...
  rows.push({ line: "" });
  s.results.forEach((r, i) => { for (const line of cachedRows(r, i === s.selected, W, worldFg, t)) rows.push({ line, i }); });
...
export function helpRows(t) {
  ...
  return [
    ...
    `   ${t.dim("keys: Alt+S stream ⇄ search · ⇧Space (or Ctrl+Space) mark agent · Ctrl+/ keyword ⇄ ai · ↑↓ agent · Shift+↑↓ result / scroll · ⏎ (empty) jump · Esc back · Tab room · ^F stream filter")}`,
  ];
}

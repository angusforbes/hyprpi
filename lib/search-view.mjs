...
export function createSearch({ api, room, onChange }) {
  const s = {
    ...
    run(query, mode = s.mode) {
      ...
      s.busy = true; s.ranFor = { q, mode, room: room() };
      s.status = mode === "ai" ? "thinking… (reading the room's recent conversations)" : "searching…";
      onChange();
      a.call("search", { room: room(), query: q, mode }, { timeoutMs: 180000 }).then((r) => {
        if (g !== s.gen) return;
    ...
    askQuestion(question) {
      ...
      a.call("ask", { room: room(), question: q }, { timeoutMs: 120000 }).then((r) => {
        if (g !== A.gen) return;
